...
def vectorize(req: dict) -> np.ndarray:
    ...
    if last is not None:
        ...
        mins_norm = _round4(
            min(max(_minutes_between(cur_time, prev_time) / 1440.0, 0.0), 1.0)
        )
        km_cur_norm = _round4(
            min(max(float(last["km_from_current"]) / 1000.0, 0.0), 1.0)
        )
    else:
        mins_norm = -1.0
        km_cur_norm = -1.0
...
def load_ivf_index(path: Path) -> dict:
    ...
    off += 12
    centroids = (
        np.frombuffer(data, dtype="<f4", count=d * k, offset=off).reshape(d, k).copy()
    )
    off += d * k * 4
    ...
    labels = np.frombuffer(
        data, dtype=np.uint8, count=total_blocks * 8, offset=off
    ).copy()
    off += total_blocks * 8
    blocks = (
        np.frombuffer(data, dtype="int8", count=total_blocks * d * 8, offset=off)
        .reshape(total_blocks, d, 8)
        .copy()
    )
...
def main() -> None:
    ...
    print(
        f"IVF loaded in {time.perf_counter() - t0:.2f}s",
        file=sys.stderr,
    )
    ...
    print(
        f"References loaded in {time.perf_counter() - t0:.2f}s",
        file=sys.stderr,
    )
    ...
    print(
        f"FAISS threads: {faiss.omp_get_max_threads()}",
        file=sys.stderr,
    )

    print(
        "Creating IndexFlatL2...",
        file=sys.stderr,
    )
    ...
    print(
        f"Index created in {time.perf_counter() - t0:.2f}s",
        file=sys.stderr,
    )
    ...
    print(
        f"Added {len(all_vecs):,} vectors in {time.perf_counter() - t0:.2f}s",
        file=sys.stderr,
    )

    print(
        "Vectorizing test entries...",
        file=sys.stderr,
    )
    ...
    print(
        f"Built {len(queries):,} queries in {time.perf_counter() - t0:.2f}s",
        file=sys.stderr,
    )
    ...
    for begin in range(0, len(queries), batch_size):
        ...
        _, bf_indices[begin:end] = fi.search(
            queries[begin:end],
            K_NEIGHBORS,
        )
        ...
        remaining = len(queries) - processed

        eta = remaining / qps if qps else 0

        print(
            f"[BF] "
            f"{processed:,}/{len(queries):,} "
            f"({processed / len(queries) * 100:.1f}%) "
            f"ETA={eta:.1f}s "
            f"QPS={qps:,.0f}",
            file=sys.stderr,
            flush=True,
    ...
    print(
        f"Brute-force search finished in {time.perf_counter() - search_start:.2f}s",
        file=sys.stderr,
    )

    bf_results = all_labels[bf_indices]
    ...
    for i, entry in enumerate(test_entries):
        if (i + 1) % 1000 == 0:
            ...
            eta = (len(test_entries) - (i + 1)) / qps

            print(
                f"[IVF] "
                f"{i + 1:,}/{len(test_entries):,} "
                f"({(i + 1) / len(test_entries) * 100:.1f}%) "
                f"ETA={eta:.1f}s",
                file=sys.stderr,
                flush=True,
            )
    ...
    print(
        f"nprobe={FAST_NPROBE} exact label match: {fast_agree}/{total} ({fast_agree / total * 100:.1f}%)"
    )
    print(
        f"nprobe={FULL_NPROBE} exact label match: {full_agree}/{total} ({full_agree / total * 100:.1f}%)"
    )
    ...
    if flip_details:
        ...
        for d in flip_details[:20]:
            print(
                f"  {d['id']}: bf={d['bf']} fast={d['fast']} full={d['full']}  "
