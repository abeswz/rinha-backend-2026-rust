...
pub static RESP_NOT_FOUND: &[u8] =
    b"HTTP/1.1 404 Not Found\r\nContent-Length: 0\r\nConnection: keep-alive\r\n\r\n";
pub static RESP_BAD_REQ: &[u8] =
...
mod tests {
    ...
    fn all_6_responses_valid_json() {
        for count in 0u8..=5 {
            ...
            assert!(
                s.contains("\"fraud_score\""),
                "count={count} missing fraud_score"
            );
        }
    }
    ...
    fn approved_flag_follows_threshold() {
        for count in 0u8..=2 {
            ...
            assert!(
                ...
                "count={count} should be approved"
            );
        }
        for count in 3u8..=5 {
            let body = std::str::from_utf8(body_of(http_body_for(count))).unwrap();
    ...
    fn content_length_matches_body() {
        for count in 0u8..=5 {
            ...
            assert_eq!(
                cl, body_len,
                "count={count}: content-length={cl} body_len={body_len}"
            );
        }
    }
